...
export default class CircleGrid extends Shader {
    constructor(shaders) {
        ...
        this.#grid.centerCircle.mat = ShaderUtils.mult2dMats(this.projectionMat, this.#grid.centerCircle.shape.mat);

        this.init(shaders).then(([circle]) => {
    ...
    #grid = (() => {
        ...
        const centerCircle = new Circle(this.gl.canvas.width / 2, this.gl.canvas.height / 2, centerCircleR, Math.PI * 2, centerCircleVertices);
        ...
        const innerCircle = new Circle(this.gl.canvas.width / 2, this.gl.canvas.height / 2, centerCircleR / 2, Math.PI * 2, centerCircleVertices);
    ...
    #initLocations() {
        this.#locations = {
            position: this.gl.getAttribLocation(this.#grid.circleProgram, "a_position"),
    ...
    #initObjectsData() {
        ...
        this.#grid.centerCircle.buffer = this.createAndBindVerticesBuffer(this.#locations.position, this.#grid.centerCircle.shape.coordinates, {
            size: 2,
        });
        ...
        this.#grid.innerCircles.buffer = this.createAndBindVerticesBuffer(this.#locations.position, this.#grid.innerCircles.shape.coordinates, {
            size: 2,
        });
    }
    ...
    #renderCenterCircle() {
        ...
        this.gl.uniformMatrix3fv(this.#locations.mat, false, this.#grid.centerCircle.mat);

        this.gl.drawArrays(this.gl.LINE_STRIP, 0, this.#grid.centerCircle.shape.verticesCount + 1);
    }
    ...
    #renderCircumferenceCircles() {
        ...
        const { count, positionAngleStep, positionCircleR } = this.#grid.circumferenceCircles;

        for (let circle = 0, positionAngle = -Math.PI / 4 - Math.PI / 2; circle < count; positionAngle += positionAngleStep, circle++) {
            ...
            this.gl.drawArrays(this.gl.LINE_STRIP, 0, this.#grid.circumferenceCircles.shape.verticesCount + 1);
        }
    }
    ...
    #renderInnerCircles() {
        ...
        const { count, positionAngleStep, positionCircleR } = this.#grid.innerCircles;
        ...
        for (
            ...
            this.gl.drawArrays(this.gl.LINE_STRIP, 0, this.#grid.innerCircles.shape.verticesCount + 1);
        }
    }

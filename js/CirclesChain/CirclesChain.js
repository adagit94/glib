...
export default class CirclesChain extends Shader {
    ...
    #initCirclesData() {
        ...
        this.#circles.circles = [{ color: [0, 0, 1] }, { color: [0, 1, 0] }, { color: [1, 0, 0] }];
        this.#circles.anims = {
            yTranslation: {
                active: true,
                direction: "down",
        ...
        this.#circles.buffers = {
            vertex: this.createAndBindVerticesBuffer(this.#circles.locations.pos, circle.coordinates, { size: 2 }),
        };
    }

    #renderCircles() {
        const { mat: circleMat, locations, program, vao, verticesCount, spacing, r, circles, anims } = this.#circles;
        ...
        for (
            ...
            xOffset + r * scale < this.gl.canvas.width && (yOffset - r * scale) * -1 < this.gl.canvas.height;
            grp++,
                xOffset += spacing.x - (xOffset / this.gl.canvas.width) * Math.pow(offsetMult, 2),
                yOffset -= spacing.y - ((yOffset * -1) / this.gl.canvas.height) * Math.pow(offsetMult, 2),
                scale = 1 - (xOffset / this.gl.canvas.width + (yOffset * -1) / this.gl.canvas.height) / 2,
                offsetMult += 1.1
        ) {
            ...
            if (this.animate && anims.yTranslation.active) {
                ...
                const limit = anims.yTranslation.limitBase + anims.yTranslation.limitAdditionFactor * grp;
                ...
                circlesGroupMat = ShaderUtils.mult2dMats(circlesGroupMat, ShaderUtils.init2dTranslationMat(0, circlesGroup.y));
            }

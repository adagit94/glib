...
export default class Snakes extends Shader {
  ...
  #head = (() => {
    ...
    const crownStripeAngleOffsetStep =
      (crownTranslationEndAngle - crownTranslationStartAngle) / crownStripes;
    ...
    left: {
      mat: ShaderUtils.mult2dMats(
        ...
        ShaderUtils.init2dTranslationMat(
          this.gl.canvas.width / 2 - this.#body.translationR,
  ...
  #initBodyData() {
    ...
    for (
      let circle = 0, r = this.#body.baseR;
      circle < this.#body.circles;
      r -= this.#body.rOffset, circle++
    ) {
      for (
        let vertex = 0, angle = 0;
        vertex < this.#body.verticesPerCircle;
        angle += this.#body.angleStep, vertex++
      ) {
  ...
  #renderBody(side) {
    ...
    for (
      let circle = 0,
        radOffset = radStep,
        translationR = this.#body.initialTranslationR;
      circle < this.#body.circles;
  ...
  #initHeadData() {
    ...
    for (
      let vertex = 0, angle = 0;
      vertex < this.#head.vertices;
      angle += this.#head.angleStep, vertex++
    ) {
    ...
    for (
      let stripe = 0,
        stripeAngleOffset = this.#head.crown.translationStartAngle;
      ...
      for (
        let vertex = 0, angle = this.#head.crown.angle;
        vertex < this.#head.crown.vertices;
        angle += this.#head.crown.angleStep, vertex++
      ) {
  ...
  #renderHead(side) {
    ...
    this.#head.mat = ShaderUtils.mult2dMats(
      this.#snakes[side].mat,
      ShaderUtils.init2dRotationMat(headRotation)
    );

    this.#head.mat = ShaderUtils.mult2dMats(
      this.#head.mat,
      ShaderUtils.init2dTranslationMat(headX, 0)
    );
    ...
    for (
      ...
      const stripeMat = ShaderUtils.mult2dMats(
        this.#head.mat,
        ShaderUtils.init2dTranslationMat(0, -y)
      );
      ...
      this.gl.drawArrays(
        this.gl.LINE_STRIP,
        stripe * this.#head.crown.vertices,
        this.#head.crown.vertices - 1
      );
  ...
  #initEyeData() {
    ...
    for (
      let vertex = 0, angle = 0, radiiStep = 0;
      vertex < this.#eye.vertices;
      angle += this.#eye.angleStep, vertex++
    ) {
  ...
  #renderEye() {
    this.#eye.mat = ShaderUtils.mult2dMats(
      this.#head.mat,
      ShaderUtils.init2dTranslationMat(0, 0)
    );

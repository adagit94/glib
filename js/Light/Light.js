...
export default class Light extends Shader {
        ...
        this.#effects = {
            pendulumTick: (() => {
                ...
                return (triangle, lightness, side) => {
                    ...
                    const fullTick = trianglesLightness.length === triangles && trianglesLightness.every((lightness) => lightness <= 0);

                    if (fullTick) {
                        if ((sides.length > 1 && side === sides[sides.length - 1]) || sides.length === 1) {
                            mode[1].direction = mode[1].direction === "forwards" ? "backwards" : "forwards";
            ...
            pulsingLightness: (lightness, anim, lastShape, lightnessBorder, stepFurther, settings) => {
                ...
                if (anim.stepping.active && stepFurther !== undefined) {
                    if (stepFurther) {
                        const outlineShape = Object.prototype.hasOwnProperty.call(anim.stepping, "range");

                        if (outlineShape) {
                            anim.stepping.nextShape += anim.stepping.range - 1 + anim.stepping.step;
                        } else {
            ...
            fluidLayers: (shapesCount, anim) => {
                ...
                if ((anim.op === "subtract" && newCount === firstShape) || (anim.op === "add" && newCount === shapesCount)) {
                    anim.deltaT = 0;
    ...
    #initRadialLightData(shapeType, conf) {
        ...
        switch (shapeType) {
            ...
            case "ellipse":
                shape = new Ellipse(conf.x, conf.y, conf.rx, conf.ry, conf.angle, conf.vertices);
                break;
        }
    ...
    #renderRadialLight() {
        ...
        for (let shape = 0, rScale = 1, lightness = inversedMode.active || striped.active ? 1 : lightnessStep; shape < count; ) {
            const mat = ShaderUtils.mult2dMats(this.#radial.mat, ShaderUtils.init2dScaleMat(rScale, rScale));
            ...
            if (this.#animate) {
                ...
                if (pulsingLightness.active) {
                    const { performPurePulsing, performStepPulsing } = PulsingUtils.getAnimsStates(anim.pulsingLightness, shape);

                    if (performPurePulsing || performStepPulsing) {
                        const isLastShape = PulsingUtils.lastShapeReached(anim.pulsingLightness, shape, count, { performStepPulsing });
                        ...
                        l = this.#effects.pulsingLightness(l, pulsingLightness, isLastShape, values.lightnessBorder, values.stepFurther, {
            ...
            this.gl.drawArrays(this.gl.LINE_STRIP, 0, this.#radial.shape.verticesCount + 1);

            if (striped.active && shape === striped.currentStripeStart + striped.width - 1) {
                ...
                if (!striped.active) {
                    lightness = inversedMode.active ? lightness - lightnessStep * lStepMult : lightness + lightnessStep * lStepMult;
                }
    ...
    #initRadialQuadData() {
        const vao = this.gl.createVertexArray();
        const circle = new Circle(this.gl.canvas.width / 2, this.gl.canvas.height / 2, this.gl.canvas.width / 4, 2 * Math.PI, 100);
    ...
    #renderRadialQuad() {
        ...
        for (let direction = 0; direction < directions; direction++) {
            for (let circle = 0, rScale = 1, lightness = lightnessStep; circle < count; rScale -= rScaleStep, lightness += lightnessStep, circle++) {
                ...
                if (this.#animate) {
                    ...
                    if (pulsingLightness.active) {
                        const { performPurePulsing, performStepPulsing, performCascadePulsing } = PulsingUtils.getAnimsStates(
                            anim.pulsingLightness,
                            circle
                        );

                        if (performPurePulsing || performStepPulsing || performCascadePulsing) {
                            const lastCircleInDirection = PulsingUtils.lastShapeReached(anim.pulsingLightness, circle, count, {
                                performStepPulsing,
                ...
                this.gl.uniformMatrix3fv(this.#locations.mat, false, mat);
                this.gl.uniform1f(this.#locations.lightness, l);
                this.gl.drawArrays(this.gl.LINE_STRIP, 0, this.#radialQuad.circle.verticesCount + 1);
            }
        }
    }
    ...
    #initTriangularStarData() {
        ...
        const indices = new Uint16Array([0, 1, 3, 2, 3, 1, 0, 1, 4, 1, 2, 5, 2, 3, 6, 3, 0, 7]);

        const centerMat = ShaderUtils.mult2dMats(
            this.projectionMat,
            ShaderUtils.init2dTranslationMat(this.gl.canvas.width / 2, this.gl.canvas.height / 2)
        );

        let squareMats = [centerMat];

        for (let square = 1, rotation = squareRotationStep; square < squares; rotation += squareRotationStep, square++) {
            squareMats.push(ShaderUtils.mult2dMats(centerMat, ShaderUtils.init2dRotationMat(rotation)));
        }
    ...
    #renderTriangularStar() {
        const { anim, vao, mats, squares, buffers, inversedMode, lStepMult } = this.#triangularStar;

        let { trianglesCount } = this.#triangularStar;

        if (this.#animate && anim.fluidLayers.active) {
            trianglesCount = this.#effects.fluidLayers(trianglesCount, anim.fluidLayers);
        }
        ...
        for (let square = 0; square < squares; square++) {
            ...
            if (this.#animate && anim.rotation.active) {
                anim.rotation.angle -= this.animData.frameDeltaTime * anim.rotation.speed;

                squareMat = ShaderUtils.mult2dMats(squareMat, ShaderUtils.init2dRotationMat(anim.rotation.angle));
            }

            for (let side = 0; side < 4; side++) {
                for (
                    let triangle = 0, scale = 1, lightness = inversedMode.active ? 1 : lightnessStep;
                    triangle < trianglesCount;
                    scale -= scaleStep,
                        lightness = inversedMode.active ? lightness - lightnessStep * lStepMult : lightness + lightnessStep * lStepMult,
                        triangle++
                ) {
                    const triangleMat = ShaderUtils.mult2dMats(squareMat, ShaderUtils.init2dScaleMat(scale, scale));
                    ...
                    if (this.#animate && anim.pulsingLightness.active) {
                        ...
                        if (performPurePulsing || performStepPulsing || performCascadePulsing) {
                            ...
                            const lastTriangleInSideReached = PulsingUtils.lastShapeReached(anim.pulsingLightness, triangle, trianglesCount, {
                                performStepPulsing,
                                performCascadePulsing,
                            });
                            ...
                            l = this.#effects.pulsingLightness(l, anim.pulsingLightness, lastTriangle, values.lightnessBorder, values.stepFurther, {
                                subanim: performStepPulsing || performCascadePulsing,
                                inverted: inversedMode.active,
                            });
                    ...
                    this.gl.drawElements(this.gl.TRIANGLES, 3, this.gl.UNSIGNED_SHORT, 3 * 2);

                    this.gl.drawElements(this.gl.TRIANGLES, 3, this.gl.UNSIGNED_SHORT, (6 + side * 3) * 2);
                }
            }
        }
    }
    ...
    #initEllipticQuadData() {
        ...
        const ellipse = new Ellipse(0, this.gl.canvas.height / 2, rx, ry, Math.PI * 2, 1000);
        ...
        this.#ellipticQuad = {
            vao,
            ellipse,
            horizontalScaleToAdd: (rx - Math.cos(Math.PI / 4) * rx) / (this.gl.canvas.width / 2),
            countPerEllipse: 750,
            directions,
            mats: [
                ellipseMat,
                ShaderUtils.mult2dMats(ellipseMat, [ShaderUtils.init2dTranslationMat(this.gl.canvas.width, 0)]),
    ...
    #renderEllipticQuad() {
        const { anim, countPerEllipse, vao, directions, mats, horizontalScaleToAdd } = this.#ellipticQuad;
        ...
        for (let direction = 0; direction < directions; direction++) {
            ...
            for (
                ...
                const scaledEllipseMat = ShaderUtils.mult2dMats(baseMat, ShaderUtils.init2dScaleMat(rScale + horizontalScaleToAdd, rScale));
                let l = lightness;
                ...
                if (this.#animate) {
                    ...
                    if (pulsingLightness.active) {
                        ...
                        if (performPurePulsing || performStepPulsing || performCascadePulsing) {
                            const lastEllipseInDirection = PulsingUtils.lastShapeReached(anim.pulsingLightness, ellipse, countPerEllipse, {
                                performStepPulsing,
                                performCascadePulsing,
                            });
                ...
                this.gl.uniform1f(this.#locations.lightness, l);
                this.gl.drawArrays(this.gl.LINE_STRIP, 0, this.#ellipticQuad.ellipse.verticesCount + 1);
            }
        }
    }
    ...
    #initTriangleLines() {
        ...
        const widthStep = (eagle ? this.gl.canvas.width / 2 : this.gl.canvas.width) / sideCount;
        ...
        if (eagle) {
            ...
            rightSideCoordinates.push(this.gl.canvas.width / 2, this.gl.canvas.height);
        } else {
            if (renderLeftSide) leftSideCoordinates.push(0, this.gl.canvas.height);
            if (renderRightSide) rightSideCoordinates.push(this.gl.canvas.width, this.gl.canvas.height);
        }

        if (renderLeftSide) {
            for (let vertex = 0, x = eagle ? 0 : this.gl.canvas.width, y = this.gl.canvas.height; vertex < vertices; vertex++) {
        ...
        if (renderRightSide) {
            for (let vertex = 0, x = eagle ? this.gl.canvas.width : 0, y = this.gl.canvas.height; vertex < vertices; vertex++) {
        ...
        this.#triangular = {
            ...
            indices: leftIndices,
            triangles,
            lightnessBase: lightness,
            lightnessStep: lightness / (triangles - 1),
    ...
    #renderTrianglesForwards = (side) => {
        ...
        this.gl.bindBuffer(this.gl.ELEMENT_ARRAY_BUFFER, this.#triangular.buffers.indices[side]);

        for (let triangle = 0, lightness = 0; triangle < triangles; lightness += lightnessStep, triangle++) {
    ...
    #renderTrianglesBackwards = (side) => {
        const { triangles, lightnessBase, lightnessStep, anim, mode } = this.#triangular;
        const { lightnessMult } = mode[1];

        this.gl.bindVertexArray(this.#triangular.vaos[side]);
        this.gl.bindBuffer(this.gl.ELEMENT_ARRAY_BUFFER, this.#triangular.buffers.indices[side]);

        for (let triangle = 0, lightness = lightnessBase; triangle < triangles; lightness -= lightnessStep, triangle++) {
            let lightnessVal = lightness;
    ...
    #renderTrianglesPendulum = (side) => {
        const { triangles, mode, lightnessBase, lightnessStep, anim } = this.#triangular;

        const { direction, triggerDimming, triangleMult, lightnessMult, sides } = mode[1];

        if (!triggerDimming) {
            const roundedT = Math.round(anim.deltaT * triangleMult);

            let frontTriangle = direction === "forwards" ? 0 + roundedT : triangles - 1 - roundedT;
        ...
        for (let triangle = 0; triangle < triangles; triangle++) {
            ...
            } else if ((direction === "forwards" && triangle > frontTriangle) || (direction === "backwards" && triangle < frontTriangle)) {
                lightness = 0;
            } else if ((direction === "forwards" && triangle < frontTriangle) || (direction === "backwards" && triangle > frontTriangle)) {
                const triangleStepsFromFront = direction === "backwards" ? triangle - frontTriangle : frontTriangle - triangle;

                lightness = lightnessBase - triangleStepsFromFront * lightnessStep;
            }
            ...
            if (triggerDimming && this.#effects.pendulumTick(triangle, lightness, side)) {
                return;
            }
        }
        ...
        if (!triggerDimming) {
            switch (direction) {
                case "forwards":
                    if (frontTriangle === triangles - 1 && ((sides.length > 1 && side === sides[sides.length - 1]) || sides.length === 1)) {
                        anim.deltaT = 0;
                ...
                case "backwards":
                    if (frontTriangle === 0 && ((sides.length > 1 && side === sides[sides.length - 1]) || sides.length === 1)) {
                        anim.deltaT = 0;

import Memory from '../behaviour/Memory.js';
...
class Shape {
  ...
  perVertexOps = false;
  mats = {
    origin: MatUtils.identity3d(),
    effect: MatUtils.identity3d(),
    offset: MatUtils.identity3d(),
    orientation: MatUtils.identity3d(),
  };
  #memory = new Memory()
  #deltaFactors = { behaviour: 0.1 };
  #effects;
  #geometryData;
  #buffers;
  ...
  setUniforms(uniforms) {
    if (Object.prototype.hasOwnProperty.call(uniforms, 'modelMat')) {
      this.perVertexOps
        ? delete uniforms.modelMat
        : this.setPosition(uniforms.modelMat);
    }
  ...
  getDeltaFactor(name) {
    return this.#deltaFactors[name];
  }

  setDeltaFactor(name, value) {
    this.#deltaFactors[name] = value;
  }

  effectsActive() {
    Object.values(this.#effects).some(effect => effect.active);
  }

  #defaultVertexHandler = vertex =>
    MatUtils.multVertWithMats3d(vertex, this.mats.offset);

  transposeVertices = (vertexHandler = this.#defaultVertexHandler) => {
    let farthestVertices = [[], [], []];

    const newVerticesCoords = (this.#geometryData.vertices =
      ShapeUtils.redefineCoords(
        this.#geometryData.vertices,
        (vertex, vertexI) => {
    ...
    const newNormalsCoords = (this.#geometryData.normals =
      ShapeUtils.computeTrianglesNormals(
        this.#geometryData.initialData.normals,
        newVerticesCoords
      ));

    this.position =
      ShapeUtils.getCuboidCenterFromFarthestVertices(farthestVertices);
  ...
  sortIndices(refPoint) {
    const sortedIndices = (this.#geometryData.indices =
      ShapeUtils.sortTriangleIndices(
        refPoint,
  ...
  render = () => {
    ...
    this.gl.drawElements(
      this.gl.TRIANGLES,
      this.#geometryData.indices.length,

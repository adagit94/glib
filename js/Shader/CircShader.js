...
export default class CircShader extends Shader {
  ...
  #initObjectsData() {
    this.#circ.mat = ShaderUtils.init2dTranslationMat(
    ...
    Object.keys(this.#circ.structures).forEach(structure => {
      ...
      this.#circ.structures[structure].buffer =
        this.createAndBindVerticesBuffer(
  ...
  #getCircleCurvesCoordinates = () => {
    ...
    for (
      let point = 0, r = baseR;
      point < CircShader.#pointsInCircleCurveGrp;
      r -= rOffsetStep, point++
    ) {
      for (
        let i = 0, rad = radStep;
        i < CircShader.#verticesPerCircleCurvePoint;
        rad += radStep, i++
      ) {
  ...
  #renderCircleCurvesGrps = () => {
    ...
    for (let grp = 0; grp < CircShader.#circGrps; grp++) {
      for (
        let point = 0, radOffset = 0;
        point < CircShader.#pointsInCircleCurveGrp;
        radOffset += radOffsetStep, point++
      ) {
        ...
        this.gl.uniformMatrix3fv(
          this.#circ.locations.mat,
          false,
          ShaderUtils.mult2dMats(this.#circ.projectionMat, pointTranslationMat)
        );
        this.gl.drawArrays(
  ...
  #getPointsCoordinates = () => {
    ...
    for (
      ...
      for (
        let i = 0, rad = 0;
        i < this.#circ.structures.points.verticesPerPoint;
        rad += this.#circ.structures.points.angleStep, i++
      ) {
        const x = Math.cos(rad) * r;
        const y = Math.sin(rad) * r;
  ...
  #renderPointsGrps = () => {
    ...
    for (let grp = 0; grp < 1; grp++) {
      ...
      for (
        ...
        point < this.#circ.structures.points.points;
        radOffset += this.#circ.structures.points.spiralAngleStep,
          r -= this.#circ.structures.points.circROffsetStep,
          point++
      ) {
        const pointRad = rad - radOffset;
  ...
  #getLinesCoordinates = variant => {
    ...
    for (let grp = 0; grp < CircShader.#circGrps; grp++) {
      for (
        let line = 0, arcLength = angle;
        line < CircShader.#arcLines;
        arcLength -= lineRadOffsetStep, line++
      ) {
        const radStep = arcLength / CircShader.#verticesPerLine;

        for (
          let vert = 0, rad = variant === "inversed" ? -Math.PI / 4 : 0;
          vert < CircShader.#verticesPerLine;
          rad += radStep, vert++
        ) {
          let x = Math.cos(rad);
          let y = Math.sin(rad);
  ...
  #renderLines = variant => {
    ...
    for (let grp = 0; grp < CircShader.#circGrps; grp++) {
      for (let line = 0; line < CircShader.#arcLines; line++) {
        ...
        const sceneMat = ShaderUtils.mult2dMats(
          this.#circ.projectionMat,
          ShaderUtils.init2dRotationMat(this.#animData.pointRotation)
        );

        this.gl.uniformMatrix3fv(
          this.#circ.locations.mat,
          false,
          ShaderUtils.mult2dMats(sceneMat, lineMat)
        );

        this.gl.uniform1i(this.#circ.locations.side, grp);

        const vertexOffset =
          grp * CircShader.#arcLines * CircShader.#verticesPerLine +
          line * CircShader.#verticesPerLine;

        this.gl.drawArrays(
          this.gl.LINE_STRIP,
          vertexOffset,
          CircShader.#verticesPerLine - 1
        );
      }
    }
  };
  ...
  #renderCircles = () => {
    const { count, vertices, vao, offsetAngle, fullCircle, rOffset, baseR } =
      this.#circ.structures.circles;
    ...
    if (!fullCircle) {
      sceneMat = ShaderUtils.mult2dMats(
        sceneMat,
        ShaderUtils.init2dTranslationMat(0, this.gl.canvas.height / 2)
      );
    }
